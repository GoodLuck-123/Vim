...
def train_segmentor(model,
    ...
    if distributed:
        ...
        # CPU-only training: skip wrapping and .cuda() if no CUDA devices available
        if torch.cuda.is_available() and len(cfg.gpu_ids) > 0:
            model = MMDataParallel(
                model.cuda(cfg.gpu_ids[0]), device_ids=cfg.gpu_ids)
        # For CPU training, don't wrap - use the model directly
        # model remains on CPU
